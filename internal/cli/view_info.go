...
func (m Model) renderStatus() string {
	...
	lines = append(lines, "  "+labelStyle.Render(fmt.Sprintf(" %-14s", t(lang, "http_port")+":"))+" "+conf.AppConfig.Port[1:])
	if conf.AppConfig.EnableFTP {
		lines = append(lines, "  "+labelStyle.Render(fmt.Sprintf(" %-14s", t(lang, "ftp_port")+":"))+" "+conf.AppConfig.FTPPort[1:])
	}
