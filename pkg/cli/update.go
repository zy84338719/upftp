...
func (m Model) handleConfigHTTPKeys(msg tea.KeyMsg) (tea.Model, tea.Cmd) {
	switch strings.ToLower(msg.String()) {
	case "y":
		conf.AppConfig.HTTPAuth.Enabled = !conf.AppConfig.HTTPAuth.Enabled
		m.message = greenStyle.Render("http auth → " + fmt.Sprintf("%v", conf.AppConfig.HTTPAuth.Enabled))
		m.view = ViewConfigMenu
		m.prevView = ViewMain
		return m, nil
	case "n", "esc", "b":
...
func (m Model) handleConfigPortsKeys(msg tea.KeyMsg) (tea.Model, tea.Cmd) {
	...
	case "enter":
		...
		if m.configStep == 0 {
			if val != "" {
				conf.AppConfig.SetHTTPPort(val)
